 ...
 * the actual outbound fetch to Resend never fires for a demo firm, not
 * just that a flag is checked -- spies on globalThis.fetch and asserts
 * zero calls to Resend's API for a demo_locked firm, at least one for a
 * real firm doing the identical action. RESEND_API_KEY is passed as a
...
function resendCallCount(spy: ReturnType<typeof mockResendOk>): number {
...
describe("DEMO-4: no outbound email ever reaches SendGrid for a demo-locked firm", () => {
  it("Add Staff (handleFirmLicenseCreate) -- the sharpest path, one step, no PATCH", async () => {
    ...
    expect(resendCallCount(fetchSpy)).toBe(0);
    ...
    expect(resendCallCount(fetchSpy)).toBe(1);
  });
